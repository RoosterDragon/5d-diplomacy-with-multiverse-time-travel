    ...
    public async Task<ActionResult<Game>> LoadGame([FromForm] GameLoadRequest request)
    {
        ...
        async Task AddOrders(int gameId)
        {
            ...
            await worldRepository.AddOrdersForMultipleIterations(gameId, saveFile.Iteration, world =>
            {
                ...
                var activePhaseByTimeline = world.ActiveBoards.ToDictionary(b => b.Timeline, b => (b.Year, b.Phase));
                var currentOrders = saveFile.Orders
                    .Skip(offset)
                    .TakeWhile(order =>
                        order.IsRetreat() == hasRetreats &&
                        activePhaseByTimeline.TryGetValue(order.Location.Timeline, out var active) &&
                        (order.Location.Year < active.Year ||
                        order.Location.Year == active.Year && order.Location.Phase <= active.Phase))
                    .ToList();
                offset += currentOrders.Count;
